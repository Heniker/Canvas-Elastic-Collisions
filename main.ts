...
class Vector {
  ...
  rotate(theta: number) {
    return new Vector(
      ...
      this.x * Math.sin(theta) - this.x * Math.cos(theta)
    )
  }
...
abstract class BaseObject implements DrawableI {
  position: Vector
  velocity: Vector
  color: string | CanvasGradient | CanvasPattern
  mass: number
  ...
  constructor({ position, velocity, color, mass }: BaseObjectContructorI) {
...
class Circle extends BaseObject {
  ...
  static bounce(circle1: Circle, circle2: Circle) {
    const it = circle1
    const that = circle2
  ...
  detectCollision(it: Circle): number | false {
